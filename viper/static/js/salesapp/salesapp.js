...
(function ($) {
	...
	window.SalesAppView = Backbone.View.extend({
		...
		addLineItem: function () {
			if (!this.model) {
				this.newOrder(function(){$('#add-item').trigger('click');});
				return;
			}
			...
			$('input[name=customername]',$('#checkoutOrderModel')).typeahead({
				...
				ajax: {
					...
					preProcess: function (data) {
						...
						if (typeof (data.mylist) == "string") 
							data.mylist = eval('(' + data.mylist + ')');
						// We good!
						return data.mylist;
					}
				}
		...
		payOrder: function() {
			...
			if(customerid) {
				...
				var orderamount = salesappview.model.get('orderamount');

				if(paymenttype == 'Cash' && paidamount >= orderamount) {
					...
					alert('Paid amount show be greater than or equal to '+ orderamount);
					return;
				}
			}else{
	...
	function listen(e) {
		...
		switch(e.keyCode){
			case 114: //new order
				...
				break;
			case 115: //cancel order
				isHandled=true
				$('#cancel-order').trigger('click');
				break;
			case 116: //checkout order
				isHandled=true
				$('#checkout-order').trigger('click');
				break;
			case 122: //print order
				isHandled=true
				$('#print-order').trigger('click');
				break;
			case 123: //preview order
				isHandled=true
