...
(function ($) {
	...
	LineItemCollection = Backbone.Collection.extend({
		...
		updateTotal: function() {
			...
			$('#totalItemsQuantity').text(this.length + '/' + qas.totalQuantity);
		},
		getQAS: function(){
			var tq = 0;
			var amt = 0;
			...
			return {totalQuantity:tq,totalAmount:amt,savings:savings};
		},
	...
	Order = Backbone.Model.extend({
		...
		getOrderAmount: function(){
			return this.get('lineItems').reduce(function(m,x){return m+x.get('subtotal');},0);
		},
		getPaidAmount: function(){
			return this.get('payments').reduce(function(m,x){return m+x.get('paidamount');},0);
		}
	});
	...
	OrdersCollection = Backbone.Collection.extend({
		...
		updateorderitem: function(orderid){
			if(!orderid) return;
			var item = this.find(function(x){return x.get('orderid')==orderid;});
			if(item) {
		...
		refreshOrders: function () {
			...
			$.ajax({
				...
				if(data && data.length>0){
					for(item=data.length-1; item>=0 ; --item) {
	...
	$('#tblTodayOrders tbody tr td a').live('click',function(e){
		...
		if(orderid)
			salesappview.editOrder(orderid);
	});
})(jQuery);
